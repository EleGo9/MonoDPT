...
def get_image_index_str(img_idx, filename_format):
    return filename_format % img_idx
    # return f'{filename_format}'.format(img_idx)
...
def get_kitti_info_path(idx,
                        ...
                        relative_path=True,
                        filename_format='%06d'):
    img_idx_str = get_image_index_str(idx, filename_format)
...
def get_image_path(idx, prefix, training=True, relative_path=True, filename_format='%06d'):
    return get_kitti_info_path(idx, prefix, 'image_2', '.png', training,
                               relative_path, filename_format='%06d')


def get_label_path(idx, prefix, training=True, relative_path=True, filename_format='%06d'):
    return get_kitti_info_path(idx, prefix, 'label_2', '.txt', training,
                               relative_path, filename_format='%06d')


def get_velodyne_path(idx, prefix, training=True, relative_path=True,filename_format='%06d'):
    return get_kitti_info_path(idx, prefix, 'velodyne', '.bin', training,
                               relative_path,filename_format='%06d')


def get_calib_path(idx, prefix, training=True, relative_path=True, filename_format='%06d'):
...
def get_kitti_image_info(path,
    ...
    def map_func(idx):
        ...
        if label_info:
            label_path = get_label_path(idx, path, training, relative_path,filename_format)
            if relative_path:
                label_path = str(root_path / label_path)
            annotations = get_label_anno(label_path,filename_format)
        if calib:
            calib_path = get_calib_path(
                idx, path, training, relative_path=False, filename_format=filename_format)
...
def get_label_annos(label_folder, image_ids=None, filename_format='%06d'):
    if image_ids is None:
        filepaths = pathlib.Path(label_folder).glob('*.txt')

        # Create regex pattern from filename_format
        # Extract the number of digits from format like '%06d', '%015d', etc.
        import re as re_module
        format_match = re_module.match(r'%0?(\d+)d', filename_format)
        if format_match:
            num_digits = int(format_match.group(1))
            # Create regex pattern: ^\d{N}.txt$ where N is the number of digits
            pattern = rf'^\d{{{num_digits}}}\.txt$'
        else:
            # Fallback: accept any number of digits
            pattern = r'^\d+\.txt$'
    ...
    for idx in image_ids:
        image_idx = get_image_index_str(idx, filename_format)
