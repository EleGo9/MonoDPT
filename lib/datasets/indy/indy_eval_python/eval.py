...
def get_indy_eval_result(gt_annos, dt_annos, current_classes, max_depth, iou_thres=0.5, PR_detail_dict=None):
    ...
    mAP_2d, mAPbev, mAP3d = do_eval_kitti(
        gt_annos, dt_annos, current_classes, min_overlaps, max_depth, False, PR_detail_dict=PR_detail_dict, )
...
def do_eval_kitti(gt_annos,
    ...
    difficultys = [0]
    ret = eval_class_kitti(gt_annos, dt_annos, current_classes, difficultys, 0, min_overlaps, max_depth)
    mAP_2d = get_mAP(ret["precision"])
    if PR_detail_dict is not None:
        PR_detail_dict['2d'] = ret['precision']

    ret = eval_class_kitti(gt_annos, dt_annos, current_classes, difficultys, 1, min_overlaps,max_depth)
    mAP_bev = get_mAP(ret["precision"])
    if PR_detail_dict is not None:
        PR_detail_dict['bev'] = ret['precision']

    ret = eval_class_kitti(gt_annos, dt_annos, current_classes, difficultys, 2, min_overlaps, max_depth)
    mAP_3d = get_mAP(ret["precision"])
    if PR_detail_dict is not None:
        PR_detail_dict['3d'] = ret['precision']
    return mAP_2d, mAP_bev, mAP_3d
...
def eval_class_kitti(gt_annos,
    ...
    for m, current_class in enumerate(current_classes):
        for l, difficulty in enumerate(difficultys):  # noqa: E741
            rets = _prepare_data(gt_annos, dt_annos, current_class, difficulty, max_depth)
            (gt_datas_list, dt_datas_list, ignored_gts, ignored_dets,
             dontcares, total_dc_num, total_num_valid_gt) = rets
            for k, min_overlap in enumerate(min_overlaps[:, metric, m]):
                thresholdss = []
    ...
    for num_part in split_parts:
        ...
        if metric == 0:
            ...
            overlap_part = d3_box_overlap(gt_boxes, dt_boxes).astype(
                np.float64)
            # print('d3 box overlap', overlap_part)
        else:
...
def _prepare_data(gt_annos, dt_annos, current_class, difficulty, max_depth):
    gt_datas_list = []
    dt_datas_list = []
    total_dc_num = []
    ...
    for i in range(len(gt_annos)):
        rets = clean_data_kitti(gt_annos[i], dt_annos[i], current_class, difficulty, max_depth)
...
def clean_data_kitti(gt_anno, dt_anno, current_class, difficulty, max_depth):
    """Clean KITTI data."""
    CLASS_NAMES = ['car', 'pedestrian', 'cyclist', 'van', 'person_sitting', 'truck']
    if not isinstance(current_class, str):
        current_class = CLASS_NAMES[current_class].lower()
    ...
    for i in range(num_dt):
        ...
        if valid_class == 1:
            ...
            ignored_dt.append(-1)
    # print('NUM valid gt', num_valid_gt)
