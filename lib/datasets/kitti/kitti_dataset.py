...
class KITTI_Dataset(data.Dataset):
    def __init__(self, split: str, cfg: Config):
        ...
        self.use_3d_center = cfg.dataset.use_3d_center #cfg.get('use_3d_center', True)
        self.writelist = cfg.dataset.writelist #cfg.get('writelist', ['Car'])
        # anno: use src annotations as GT, proj: use projected 2d bboxes as GT
        self.bbox2d_type = cfg.dataset.bbox2d_type #cfg.get('bbox2d_type', 'anno')
        assert self.bbox2d_type in ['anno', 'proj']
        self.meanshape = cfg.dataset.meanshape #cfg.get('meanshape', False)
        self.class_merging = cfg.dataset.class_merging #cfg.get('class_merging', False)
        self.use_dontcare = cfg.dataset.use_dontcare #cfg.get('use_dontcare', False)
        ...
        self.idx_list = [x.strip() for x in open(self.split_file).readlines()] #[:32] #todo: solo per debu, ricordare di levare
        ...
        self.aug_pd = cfg.dataset.aug_pd #cfg.get('aug_pd', False)
        self.aug_crop = cfg.dataset.aug_crop #cfg.get('aug_crop', False)
        self.aug_calib = cfg.dataset.aug_calib #cfg.get('aug_calib', False)

        self.random_mixup3d = cfg.dataset.random_mixup3d #cfg.get('random_mixup3d', 0.5)
        self.random_flip = cfg.dataset.random_flip #cfg.get('random_flip', 0.5)
        self.random_crop = cfg.dataset.random_crop #cfg.get('random_crop', 0.5)
        self.scale = cfg.dataset.scale #cfg.get('scale', 0.4)
        self.shift = cfg.dataset.shift #cfg.get('shift', 0.1)

        self.depth_scale = cfg.dataset.depth_scale #cfg.get('depth_scale', 'normal')
        self.min_conf = cfg.tester.threshold #cfg.get('min_conf', 0.1)
        # statistics
        ...
        self.clip_2d = cfg.dataset.clip_2d #cfg.get('clip_2d', False)
    ...
    def eval(self, results_dir, logger):
        error_avg = get_MAE(results_folder = results_dir, gt_folder= self.label_dir, conf = self.min_conf, use_logging= True, logger= logger, visualize=True)
