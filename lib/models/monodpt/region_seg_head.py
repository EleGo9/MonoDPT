...
class RegionSegHead(nn.Module):
    ...
    def forward(self, features):
        ...
        for i in range(len(features) - 2, -1, -1):
            # p[i] = self.input_proj[i](features[i]) + self.upsample(p[i + 1]) # orig

            # --- ele ---
            upsampled = self.upsample(p[i + 1])
            current_feat = self.input_proj[i](features[i])

            # Ensure spatial dimensions match by interpolating if needed
            if upsampled.shape[-2:] != current_feat.shape[-2:]:
                upsampled = F.interpolate(upsampled, size=current_feat.shape[-2:],
                                          mode='bilinear', align_corners=True)

            p[i] = current_feat + upsampled
