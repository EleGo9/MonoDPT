...
from lib.models.depth_anything_v2.dpt import DPTHead

class DepthPredictorWithDPT(nn.Module):
    """
    Depth Predictor using Depth Anything V2's DPT head
    This version uses features from shared DINOv2 backbone
    """

    def __init__(self, cfg: Config):
        """
        Initialize depth predictor with DPT head and depth encoder
        Args:
            model_cfg [EasyDict]: Depth classification network config
        """
        super().__init__()

        depth_num_bins = int(cfg.model.num_depth_bins)
        depth_min = float(cfg.model.depth_min)
        depth_max = float(cfg.model.depth_max)
    ...
    def forward(self, feature, mask, pos, depth_map, target_size=None):
        """
        Args:
            feature: Multi-scale features from backbone (list of 3 feature maps)
            mask: Attention mask
            pos: Positional encoding
            backbone_features: Raw features from DINOv2 backbone (if using DPT head)
            target_size: Optional (H, W) tuple to resize metric depth before logits computation
        """

        # ---- mono dino detr
        B, _, H_full, W_full = depth_map.shape
        H_small, W_small = mask.shape[-2], mask.shape[-1] # (= patch size)
        ...
        #weighted_depth = weighted_depth_small.squeeze(1) * self.depth_max # wip
        ...
        # return weighted_depth_small, depth_embed, weighted_depth  # , depth_pos_embed_ip
