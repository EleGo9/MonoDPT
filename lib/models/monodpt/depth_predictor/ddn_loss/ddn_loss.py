...
class DDNLoss(nn.Module):
    ...
    @staticmethod
    def build_target_depth_from_3dcenter(depth_logits, gt_boxes2d, gt_center_depth, num_gt_per_img):
    ...
    def forward(self, depth_logits, gt_boxes2d, num_gt_per_img, gt_center_depth):
        ...
        if bins == 1:
            ...
            reg_loss = F.smooth_l1_loss(depth_logits.squeeze(1), depth_maps, reduction='none')
            """dp = depth_logits.squeeze(1)[0].detach().cpu().numpy()
            dt = depth_maps[0].cpu().numpy()
            cv2.imwrite("dt_fix.png", dt)"""
