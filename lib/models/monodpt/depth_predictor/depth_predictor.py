...
class DepthPredictor(nn.Module):
    ...
    def __init__(self, model_cfg):
        ...
        """
        super().__init__()
        depth_num_bins = int(model_cfg.model.num_depth_bins)
        depth_min = float(model_cfg.model.depth_min)
        depth_max = float(model_cfg.model.depth_max)
        ...
        # Create modules
        d_model = model_cfg.model.hidden_dim
        ...
        # self.depth_pos_embed = nn.Embedding(int(self.depth_max) + 1, d_model) note: unused param (wip)
    ...
    def forward(self, feature, mask, pos):
        ...
        #depth_pos_embed_ip = self.interpolate_depth_embed(weighted_depth) (wip)
        depth_embed = depth_embed #+ depth_pos_embed_ip (wip)
