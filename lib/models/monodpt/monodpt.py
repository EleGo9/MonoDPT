...
#from .depth_predictor import DepthPredictor #orig
from .depth_predictor import build_depth_predictor #ele
...
class MonoDPT(nn.Module):
    """ This is the MonoDPT module that performs monocualr 3D object detection """
    def __init__(self,
                cfg : Config,
                backbone: nn.Module, depth_predictor: nn.Module, det2d_transformer: nn.Module, det3d_transformer: nn.Module,
                num_classes: int, num_queries: int, num_feature_levels: int,
                aux_loss: bool = True, with_box_refine: bool = False, init_box: bool = False, group_num:int = 11):
        ...
        if num_feature_levels > 1:
            ...
            in_channels = backbone[0].embed_dim  # 384
            out_channels = backbone[0].num_channels  # [48, 96, 192, 384]
        ...
        num_pred = det3d_transformer.decoder.num_layers #+ 1 # note: wrong, cause unused params<<-------- (wip)
        if with_box_refine:
    ...
    def extract_features_monodgp(self, images):
        features, pos = self.backbone(images) # orig
        return features, pos

    def extract_features_monodpt(self, images):
        # -- ele --
        # Store original resolution for depth map resizing

        B, _, H_orig, W_orig = images.shape
        self.depth_target_size = (H_orig // 16, W_orig // 16)  # Adjust divisor based on depth_downsample_factor

        features, pos, extr_out = self.backbone(images)

        # Get raw DINOv2 features for DPT head (if using DINOv2 backbone)
        raw_features = extr_out["raw_features"]
        # -- /ele --

        return features, pos, raw_features

    def depth_pred_monodgp(self, srcs, masks, seg_embed, pos):
        # orig
        pred_depth_map_logits, depth_pos_embed, weighted_depth = self.depth_predictor(srcs, masks[1], seg_embed[1] + pos[1])
        return pred_depth_map_logits, depth_pos_embed, weighted_depth

    def dept_pred_monodpt(self, srcs, masks, seg_embed, pos, raw_backbone_features):
        # -- ele --
        """# Calculate target depth map size based on original resolution
        if H_orig != H_dino or W_orig != W_dino:
            depth_target_size = (H_orig // 16, W_orig // 16)  # Adjust divisor based on depth_downsample_factor
        else:
            depth_target_size = None"""

        pred_depth_map_logits, depth_pos_embed, weighted_depth = self.depth_predictor(
            srcs, masks[-1], seg_embed[-1] + pos[-1],
            depth_map=raw_backbone_features,  # Pass raw DINOv2 features for DPT head
            target_size=self.depth_target_size
        )

        """# If depth predictor resized features, also resize corresponding masks and pos
        if self.depth_target_size is not None:
            H_target, W_target = self.depth_target_size
            # Resize masks and pos for levels 0-2 to match resized features
            for lvl in range(min(3, len(srcs))):
                if srcs[lvl].shape[-2:] != masks[lvl].shape[-2:]:
                    masks[lvl] = F.interpolate(
                        masks[lvl].unsqueeze(1).float(),
                        size=(H_target, W_target),
                        mode='nearest'
                    ).squeeze(1).bool()
                    pos[lvl] = F.interpolate(
                        pos[lvl],
                        size=(H_target, W_target),
                        mode='bilinear',
                        align_corners=False
    ...
    def forward(self, images, calibs, targets, img_sizes, dn_args=None):
        ...
        if self.backbone_type == "resnet50":
            features, pos = self.extract_features_monodgp(images)
        elif self.backbone_type == "dinov2_vits" or self.backbone_type == "dinov2_vitb" or self.backbone_type == "dinov2_vitl":
            features, pos, raw_features = self.extract_features_monodpt(images)
        else:
        ...
        # -----
        if self.backbone_type == "resnet50":
            pred_depth_map_logits, depth_pos_embed, weighted_depth = self.depth_pred_monodgp(srcs, masks, seg_embed, pos)
        elif self.backbone_type == "dinov2_vits" or self.backbone_type == "dinov2_vitb" or self.backbone_type == "dinov2_vitl":
            pred_depth_map_logits, depth_pos_embed, weighted_depth = self.dept_pred_monodpt(srcs, masks, seg_embed, pos, raw_features)
        else:
            raise ValueError(f"Backbone type {self.backbone_type} not supported")
        # -----

        intermediate_output = self.det2d_transformer(srcs, masks, pos, query_embeds) # visual encoder?
        ...
        for lvl in range(hs.shape[0]): # 3D head
            if lvl == 0:
...
class SetCriterion(nn.Module):
    ...
    def loss_depth_map(self, outputs, targets, indices, num_boxes):
        ...
        gt_boxes2d = torch.cat([t['boxes'] for t in targets], dim=0) * torch.tensor([W, H, W, H], device='cuda') # torch.tensor([80, 24, 80, 24], device='cuda')
    ...
    def loss_region(self, outputs, targets, indices, num_boxes):
        ...
        for region_prob in region_probs:
            # (wip): non sono sicuro quanto sia una buona idea avere questo interpolate. Forse meglio creare le maschere GT al volo dalle box, come per la depth
    ...
    def get_loss(self, loss, outputs, targets, indices, num_boxes, **kwargs):
        ...
        loss_fn = loss_map.get(loss, None)
        if loss_fn is None:
            return {}
        return loss_fn(outputs, targets, indices, num_boxes, **kwargs)

    def forward(self, outputs, targets, mask_dict=None, fabric=None):
        ...
        """if fabric is not None:
            fabric.all_reduce(num_boxes, reduce_op="sum")
        ...
        num_boxes = torch.clamp(num_boxes / ws, min=1).item()
        losses = {}
...
def build(cfg: Config):
    ...
    #depth_predictor = DepthPredictor(cfg) # orig
    depth_predictor = build_depth_predictor(cfg) # ele
    ...
    model = MonoDPT(
        ...
        init_box=cfg.model.init_box,
        group_num=cfg.model.group_num)

    # matcher
    matcher = build_matcher(cfg)

    # loss
    weight_dict = {'loss_ce': cfg.model.cls_loss_coef, 'loss_bbox': cfg.model.bbox_loss_coef}
    weight_dict['loss_giou'] = cfg.model.giou_loss_coef
    weight_dict['loss_dim'] = cfg.model.dim_loss_coef
    weight_dict['loss_angle'] = cfg.model.angle_loss_coef
    weight_dict['loss_depth'] = cfg.model.depth_loss_coef
    weight_dict['loss_center'] = cfg.model.loss_3dcenter
    weight_dict['loss_depth_map'] = cfg.model.depth_map_loss_coef
    weight_dict['loss_region'] = cfg.model.region_loss_coef
