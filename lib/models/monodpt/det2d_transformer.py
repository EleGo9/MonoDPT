...
class VisualEncoderLayer(nn.Module):
    ...
    def forward(self, src, pos, reference_points, spatial_shapes, level_start_index, padding_mask=None):
        ...
        # workaround: deformable att do not support half (RuntimeError: "ms_deform_attn_forward_cuda" not implemented for 'Half')
        with torch.cuda.amp.autocast(enabled=False): #
            src2 = self.self_attn(
                self.with_pos_embed(src.float(), pos.float() if pos is not None else None),
    ...
    def forward(self,
        ...
        with torch.cuda.amp.autocast(enabled=False):
            tgt2 = self.cross_attn(
                self.with_pos_embed(tgt.float(), query_pos.float()),
                reference_points.float(),
                src.float(),
                src_spatial_shapes,
                level_start_index,
